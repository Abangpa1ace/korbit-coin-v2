...
function ListQuotesFilters(): JSX.Element {
  ...
  const { getParamsPerPage, getParamsVsCurrency, updateRequestParams } = useQuoteStore(
    ({ getParamsPerPage, getParamsVsCurrency, updateRequestParams }) => ({
      getParamsPerPage,
      getParamsVsCurrency,
      updateRequestParams,
    }),
  ...
  return (
    <Container>
      <Select<ListTabType>
        ...
        handleChange={handleChangeTabSelect}
      />
      <Spacer x={12} />
      <Select<ListPerPageType>
        value={getParamsPerPage()}
        options={PerPageSelectOptions}
        handleChange={handleChangePerPage}
      />
      <Spacer x={12} />
      <CurrencySelect value={getParamsVsCurrency()} onChange={handleChangeVsCurrency} />
    </Container>
  );
}
